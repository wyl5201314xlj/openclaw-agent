...
> **⚠️ 状态：已于 2026-09-06 经主人决定放弃本方向。**
> 梯子子系统（node_fetcher/node_prober/node_store/node_scheduler/isp_classifier、/sub 路由、订阅端点）已全部删除，
> 仓库保留 OpenClaw 智能体本体（QQ 网关/模型路由/ReAct/定时提醒/生图/检索）。
> 下文仅作历史记录保留。
>
> 放弃原因：手机实测全站无法连通；免费共享池经三级探活后真实可用率过低（每轮 2~6 个），
> 且存活集合每小时漂移，无法达到"稳定"预期。若未来重启该方向，优先路线是自建独享 VPS。
