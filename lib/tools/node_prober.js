  ...
  async probeAndRankNodes(candidateNodes, maxConcurrency = 5, targetCount = 20) {
    ...
    const probeSingle = async (node) => {
      ...
      }
      {
        ...
        // 兼容性过滤（国内 Clash Meta 实测）：
        // 1. vmess + network http 在移动端握手失败率极高，直接淘汰；
        // 2. vmess + alterId 非 0 且无 tls 的，服务端多已禁用，降权到最后。
        if (node.type === 'vmess' && String(node.network || '').toLowerCase() === 'http') {
          return;
        }
        let compatPenalty = 0;
        if (node.type === 'vmess' && Number(node.alterId) > 0 && !node.tls) {
          compatPenalty = -300;
        }
        ...
        const cleanNode = {
          ...
          isResidential,
          ispTag,
          scoreBonus: scoreBonus + compatPenalty,
          udp: true
        };
