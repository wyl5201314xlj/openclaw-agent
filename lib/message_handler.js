...
return async function handleMessage(content, ctx) {
    ...
    // 「详情 N」：深挖最近一期热点推送的第 N 条（复用 ReAct 联网检索）
    const detailMatch = text.match(/^详情\s*(\d{1,2})$/);
    if (detailMatch) {
      const hotnews = require('./hotnews');
      const last = await hotnews.getLastState();
      const idx = Number(detailMatch[1]) - 1;
      const titles = last.titles || [];
      if (idx < 0 || idx >= titles.length) {
        await ctx.reply(`没有找到第 ${detailMatch[1]} 条热点，今天的推送里只有 ${titles.length} 条。`);
        return;
      }
      const title = titles[idx];
      await ctx.reply(`正在检索「${title.slice(0, 40)}」…`);
      const task = await agent.processGoal(`请联网检索这条新闻的详细内容并用中文简要总结：${title}`, {
        sessionScope: ctx.sessionScope,
        targetOpenid: ctx.target,
        isGroup: ctx.isGroup,
      });
      const budget = Math.max(1, remainingReplies(ctx, gateway));
      for (const chunk of splitForChat(task.result, budget)) {
