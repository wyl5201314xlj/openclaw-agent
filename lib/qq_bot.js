...
class QQBotGateway {
  constructor() {
    ...
    this.recentSenders = []; // 最近发消息的用户 openid 环形缓冲（管理面诊断用，最多 10 条）
    this.stats = {
      received: 0,
      sent: 0,
      sendFailed: 0,
  ...
  async dispatchUserMessage(eventType, data) {
    ...
    // 记录最近发消息的用户（openid 按机器人隔离，管理面据此配置推送目标）
    if (senderId && !isGroup) {
      this.recentSenders = [
        { openid: senderId, ts: Date.now() },
        ...this.recentSenders.filter((s) => s.openid !== senderId),
      ].slice(0, 10);
    }
  ...
  snapshot() {
    return {
      ...
      recentSenders: this.recentSenders,
      stats: this.stats,
    };
  }
}
