...
class QQBotGateway {
  ...
  async onUserMessage(eventType, data) {
    ...
    // 【100% 开放：无任何认主拦截，任何人发送消息全部正常响应干活】
    ...
    if (content.includes('提醒') && (content.includes('后') || content.includes('分') || content.includes('秒') || content.includes('点'))) {
      ...
      await this.sendSmartReply(eventType, data, `⏰ 收到指令！已为您设定后台定时器：将在 ${seconds} 秒后准时主动向您推送提醒：\n👉 "${remindText}"`);
      return;
    }
    ...
    if (content.startsWith('画') || content.startsWith('生成图片') || content.includes('生图') || content.includes('画一只') || content.includes('画一张')) {
      ...
      await this.sendSmartReply(eventType, data, `🎨 收到绘画需求，正在调用 Agnes 顶级画师云端渲染中...\n【画面】: ${drawPrompt}`);

      try {
        const imgResult = await imageTool.generateImage(drawPrompt);
        if (imgResult.url) {
          await this.sendSmartReply(eventType, data, `🖼️ 画作已渲染完成！\n高清直链: ${imgResult.url}`);
        } else {
          await this.sendSmartReply(eventType, data, `🖼️ 画作已渲染完成！`);
        }
      } catch (err) {
    ...
    // 意图 C: 通用 Agent 任务与深度 ReAct 问答
    await this.sendSmartReply(eventType, data, `🧠 正在云端 7x24h 容器启动 ReAct 闭环规划与工具调用...\n【目标】: ${content}`);

    try {
      const taskRecord = await agent.processGoal(content, { senderId, isGroup });
      const resultText = taskRecord.result || '执行完毕。';

      if (resultText.length > 900) {
        await this.sendSmartReply(eventType, data, `📋 [执行报告 1/2]:\n${resultText.slice(0, 900)}`);
        await new Promise(r => setTimeout(r, 300));
        await this.sendSmartReply(eventType, data, `📋 [执行报告 2/2]:\n${resultText.slice(900)}`);
      } else {
        await this.sendSmartReply(eventType, data, `📋 执行报告：\n${resultText}`);
      }
    } catch (err) {
